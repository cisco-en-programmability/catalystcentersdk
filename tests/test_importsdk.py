...
class TestImportSDK:
    ...
    def setup(self, base_url):
        ...
        self.verify = False
        self.debug = True
        self.version = "2.3.7.6.1"


@pytest.fixture
def import_fixture(mock_catalystcenter_server, base_url):
...
def reset_env_vars(monkeypatch):
    monkeypatch.delenv("CATALYST_CENTER_BASE_URL", raising=False)
    monkeypatch.delenv("CATALYST_CENTER_USERNAME", raising=False)
    monkeypatch.delenv("CATALYST_CENTER_PASSWORD", raising=False)
    monkeypatch.delenv("CATALYST_CENTER_ENCODED_AUTH", raising=False)
